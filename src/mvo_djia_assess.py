...
# prefix = "results/djia_3yr_extra_small" # 0.1
# prefix = "results/djia_3yr_small" # 0.3
# prefix = "results/djia_3yr_middle" # 0.5
# prefix = "results/djia_3yr_large" # 0.7
# prefix = "results/djia_3yr_extra_large" # 0.9

# prefix = "results/djia_3yr1990to2020" # 0.5
# prefix = "results/djia_3yr_extra_large_full"  # 0.9
prefix = "results/djia_1990_2020_c10bps_t10bps/djia_3yr_ts_0.10"  # 0.5


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Assess S&P Performance")
    ...
    port_names_dict = {
        "10pct": "Ultra Conservative\n10%",
        "13pct": "Conservative\n13%",
        "16pct": "Moderate\n16%",
        "19pct": "Aggressive\n19%",
        "22pct": "Ultra Aggressive\n22%",
    ...
    for cov in color_maps.keys():
        risk_columns = [("%02dpct" % rsk, cov) for rsk in range(8, 25, 2)]
        ...
        if cov == "HC":
            for idx, rsk in enumerate(risk_columns):
                plt.text(stds[idx], rets[idx]-0.25, range(8, 25, 2)[idx],  verticalalignment='bottom', fontsize=12)
        plt.legend(loc="best")
    ...
    risks_columns = [("%02dpct" % rsk, cov) for rsk in [10, 13, 16, 19, 22] for cov in color_maps.keys()]
    #ports_columns = [(port, cov) for port in ['maxSharpe'] for cov in color_maps.keys()] # remove Sharpe Ratio
